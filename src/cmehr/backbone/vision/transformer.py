...
class VisionTransformerPooler(nn.Module):
    ...
    def __init__(
        ...
        self.blocks = nn.ModuleList(
            [Block(**block_kwargs) for _ in range(num_blocks)])
        ...
        pos_embed = pos_embed_class(mask=torch.ones(
            [1, grid_shape[0], grid_shape[1]]))  # 1 x L x C
    ...
    def forward(self, current_image: torch.Tensor, previous_image: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        current_patch_features = current_token_features.transpose(
            1, 2).view(B, C, H, W)
...
class MultiHeadAttentionLayer(nn.Module):
    ...
    def forward(self, k: torch.Tensor, q: torch.Tensor, v: torch.Tensor) -> MultiHeadAttentionOutput:
        ...
        w_k = self.proj_k(k).reshape(B, N, self.num_heads,
                                     C // self.num_heads).permute(0, 2, 1, 3)
        w_v = self.proj_v(v).reshape(B, N, self.num_heads,
                                     C // self.num_heads).permute(0, 2, 1, 3)
...
class Block(nn.Module):
    ...
    def forward(self, x: torch.Tensor, pos_and_type_embed: Optional[torch.Tensor]) -> torch.Tensor:
        x_with_emb = self.with_pos_and_type_embed(
            self.norm1(x), emb=pos_and_type_embed)
        x = x + \
            self.drop_path(self.attn.forward_as_mhsa(x_with_emb).mha_output)
...
class SinePositionEmbedding:
    ...
    def __call__(self, mask: torch.Tensor) -> torch.Tensor:
        ...
        dim_t = self.temperature ** (2 *
                                     torch.div(dim_t, 2, rounding_mode="floor") / self.embedding_dim)
        ...
        pos = torch.cat((pos_y, pos_x), dim=3).view(
            B, H * W, self.embedding_dim * 2)
