...
class ImageEncoder(nn.Module):
    ...
    def _create_encoder(self, **kwargs: Any) -> nn.Module:
        if self.img_encoder_type in [ImageEncoderType.RESNET18, ImageEncoderType.RESNET18_MULTI_IMAGE]:
            ...
            supported = ImageEncoderType.get_members(
                multi_image_encoders_only=False)
    ...
    def reload_encoder_with_dilation(self, replace_stride_with_dilation: Optional[Sequence[bool]] = None) -> None:
        ...
        if self.img_encoder_type == ImageEncoderType.RESNET18:
            ...
            raise NotImplementedError(
                "resnet18 does not support dilated convolutions")
        ...
        new_encoder = self._create_encoder(
            replace_stride_with_dilation=replace_stride_with_dilation).to(device)
...
class MultiImageEncoder(ImageEncoder):
    ...
    def __init__(self, img_encoder_type: str):
        ...
        # The aggregate feature dim of the encoder is `2 * output_dim` i.e. [f_static, f_diff]
        output_dim = 256
        ...
        self.missing_previous_emb = nn.Parameter(
            torch.zeros(1, output_dim, 1, 1))
