...
from .device import get_module_device
...
class ImageModel(BaseImageModel):
    ...
    def __init__(
        ...
        **downstream_classifier_kwargs: Any,
        ...
        self.feature_size = get_encoder_output_dim(
            self.encoder, device=get_module_device(self.encoder))
        self.projector = MLP(
        ...
        self.classifier = self.create_downstream_classifier(
        ) if downstream_classifier_kwargs else None
        ...
        if pretrained_model_path is not None:
            if not isinstance(pretrained_model_path, (str, Path)):
                raise TypeError(
                    f"Expected a string or Path, got {type(pretrained_model_path)}")
    ...
    def forward_post_encoder(self, patch_x: torch.Tensor, pooled_x: torch.Tensor) -> ImageModelOutput:
        with torch.set_grad_enabled(not self.freeze_encoder):
            ...
            projected_global_embedding = torch.mean(
    ...
    def get_patchwise_projected_embeddings(self, input_img: torch.Tensor, normalize: bool) -> torch.Tensor:
        ...
        projected_embeddings = projected_embeddings.permute(
            [0, 2, 3, 1])  # B D H W -> B H W D (D: Features)
...
class MultiImageModel(ImageModel):
    def __init__(self, **kwargs: Any) -> None:
        ...
        assert isinstance(
            self.encoder, MultiImageEncoder), "MultiImageModel only supports MultiImageEncoder"
