...
class PositionalEncoding2D(nn.Module):
    ...
    def forward(self, tensor):
        ...
        pos_x = torch.arange(x, device=tensor.device,
                             dtype=self.inv_freq.dtype)
        pos_y = torch.arange(y, device=tensor.device,
                             dtype=self.inv_freq.dtype)
        sin_inp_x = torch.einsum("i,j->ij", pos_x, self.inv_freq)
...
class PositionalEncoding3D(nn.Module):
    ...
    def forward(self, tensor):
        ...
        pos_y = torch.arange(y, device=tensor.device,
                             dtype=self.inv_freq.dtype)
        pos_z = torch.arange(z, device=tensor.device,
                             dtype=self.inv_freq.dtype)
        sin_inp_x = torch.einsum("i,j->ij", pos_x, self.inv_freq)
        sin_inp_y = torch.einsum("i,j->ij", pos_y, self.inv_freq)
        ...
        self.cached_penc = emb[None, :, :, :,
                               :orig_ch].repeat(batch_size, 1, 1, 1, 1)
        return self.cached_penc
...
if __name__ == "__main__":
    pos_1d = PositionalEncoding1D(channels=128)
    print(pos_1d)
    x = torch.randn(2, 64, 128)
    print(pos_1d(x).shape)
