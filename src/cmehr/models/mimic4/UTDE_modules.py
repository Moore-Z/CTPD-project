...
class Outer(nn.Module):
    ...
    def forward(self, inp1, inp2):
        ...
        fusion = torch.zeros(
            (batch_size, self.inp1_size + 1, self.inp2_size + 1)).type_as(inp1)
...
class MAGGate(nn.Module):
    ...
    def forward(self, inp1, inp2):
        ...
        alpha = torch.min(torch.norm(inp1) /
                          torch.norm(adjust) * self.beta, one)
        ...
        for model in [self.gate]:
            for layer in model:
                if type(layer) in [nn.Linear]:
                    torch.nn.init.xavier_normal_(layer.weight)  # type: ignore
...
class TransformerCrossEncoder(nn.Module):
    ...
    def __init__(self, embed_dim, num_heads, layers, attn_dropout=0.0, relu_dropout=0.0, res_dropout=0.0,
        ...
        if self.q_seq_len_2 != None:
            ...
            self.embed_positions_q = nn.ModuleList(
                [self.embed_positions_q_1, self.embed_positions_q_1,])
    ...
    def forward(self, x_in_list):
        ...
        if self.q_seq_len_1 is not None:
            position_x1 = torch.arange(length_x1).type_as(x_in_list[0]).long()
            position_x2 = torch.arange(length_x2).type_as(x_in_list[0]).long()
            positions = [position_x1, position_x2]
            x_list = [l(position_x).unsqueeze(0).transpose(
                0, 1) + x for l, x, position_x in zip(self.embed_positions_q, x_list, positions)]
...
class MultiheadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        if self.add_zero_attn:
            src_len += 1
            k = torch.cat(
                [k, k.new_zeros((k.size(0), 1) + k.size()[2:])], dim=1)  # type: ignore
            v = torch.cat(
                [v, v.new_zeros((v.size(0), 1) + v.size()[2:])], dim=1)  # type: ignore
            if attn_mask is not None:
                attn_mask = torch.cat(
                    [attn_mask, attn_mask.new_zeros(attn_mask.size(0), 1)], dim=1)

        attn_weights = torch.bmm(q, k.transpose(1, 2))  # type: ignore
...
class TransformerCrossEncoderLayer(nn.Module):
    def __init__(self, embed_dim, num_heads=4, attn_dropout=0.1, relu_dropout=0.1, res_dropout=0.1,
        ...
        self.fc1 = nn.ModuleList(
            [nn.Linear(self.embed_dim, 4*self.embed_dim) for _ in range(2)])
        self.fc2 = nn.ModuleList(
            [nn.Linear(4*self.embed_dim, self.embed_dim) for _ in range(2)])
        self.pre_ffn_layer_norm = nn.ModuleList(
            [nn.LayerNorm(self.embed_dim) for _ in range(2)])
    ...
    def forward(self, x_list):
        ...
        output = [l(query=x, key=x, value=x)
                  for l, x in zip(self.self_attns, x_list)]
        x_list = [x for x, _ in output]
        x_list[0] = F.dropout(
            x_list[0], p=self.res_dropout, training=self.training)
        x_list[1] = F.dropout(
            x_list[1], p=self.res_dropout, training=self.training)
        x_list = [r + x for r, x in zip(residual, x_list)]

        residual = x_list
        x_list = [l(x)
                  for l, x in zip(self.pre_encoder_attn_layer_norm, x_list)]
        ...
        x_list[0] = F.dropout(
            x_list[0], p=self.relu_dropout, training=self.training)
        x_list[1] = F.dropout(
            x_list[1], p=self.relu_dropout, training=self.training)
    ...
    def __init__(self, embed_dim, num_heads, layers, attn_dropout=0.0, relu_dropout=0.0, res_dropout=0.0,
        ...
        if learn_embed:
            ...
            if self.kv_seq_len != None:
                self.embed_positions_kv = nn.Embedding(
                    self.kv_seq_len, embed_dim)
    ...
    def forward(self, x_in, x_in_k=None, x_in_v=None):
        ...
        if self.q_seq_len is not None:
            position_x = torch.arange(
                length_x).type_as(x_in).long()
            x += (self.embed_positions_q(position_x).unsqueeze(0)
                  ).transpose(0, 1)  # Add positional embedding
        ...
        if x_in_k is not None and x_in_v is not None:
            ...
            position_kv = torch.tensor(torch.arange(
                length_kv)).type_as(x_in).long()

            x_k = self.embed_scale * x_in_k
            x_v = self.embed_scale * x_in_v
            if self.kv_seq_len is not None:
                x_k += (self.embed_positions_kv(position_kv).unsqueeze(0)
                        ).transpose(0, 1)   # Add positional embedding
                x_v += (self.embed_positions_kv(position_kv).unsqueeze(0)
                        ).transpose(0, 1)   # Add positional embedding
...
class TimeSeriesCnnModel(nn.Module):
    def __init__(self, input_size, n_filters, filter_size, dropout, length, n_neurons, layers):
        ...
        self.layers = layers
        if layers >= 1:
            self.conv1 = nn.Conv1d(input_size, n_filters,
                                   filter_size, padding=padding)
        ...
        if layers >= 3:
            self.conv3 = nn.Conv1d(n_filters, n_filters,
                                   filter_size, padding=padding)
